...
var Windy = function Windy(params) {
  ...
  var PARTICLE_MULTIPLIER = params.particleMultiplier || 1 / 300; //绘制粒子数量的比例（宽像素*高像素*此比例）
  ...
  //指定经纬度与A（0，0）差值 x,y ,组成矩形的A（0，0）B（1，0）C（0，1）D（1，1）四个点
  var bilinearInterpolateVector = function bilinearInterpolateVector(x, y, g00, g10, g01, g11) {
    // console.log("#####builder.interpolate-----", x, y, g00, g10, g01, g11)
    var rx = 1 - x;
    var ry = 1 - y;
    var a = rx * ry,
      b = x * ry,
  ...
  //将传入的数组数据转换为 三维数组数据  经度-纬度-uv数据 [ [[u,v],[u,v],[u,v]] , [[u,v],[u,v],[u,v]] ]
  ...
  var interpolate = function interpolate(λ, φ) {
    if (!grid) return null;
    var i = floorMod(λ - λ0, 360) / Δλ; // 得到经度[0, 360)
  ...
  var interpolateField = function interpolateField(grid, bounds, extent, callback) {
    ...
    function interpolateColumn(x) {
      ...
      for (var y = bounds.y; y <= bounds.yMax; y += 2) {
        var coord = invert(x, y);//得到X , Y 点对应地图上的经纬度
        if (coord) {
          var λ = coord[0],
            φ = coord[1];
          if (isFinite(λ)) {
